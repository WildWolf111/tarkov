...
//POST
func (api *APIServer) PostProduct(writer http.ResponseWriter, req *http.Request) {
	initHeaders(writer)
	api.logger.Info("Post Warehouse_cells POST /products")
	var Gtd models.Gtd
...
//delete
func (api *APIServer) DeleteProductById(writer http.ResponseWriter, req *http.Request) {
	initHeaders(writer)
	api.logger.Info("Delete GtdById DELETE /products/delete/{id}")
...
func (api *APIServer) GetAllProducts(writer http.ResponseWriter, req *http.Request) {
	initHeaders(writer)
	...
	api.logger.Info("Get All products GET /gtd")
...
func (api *APIServer) GetProductById(writer http.ResponseWriter, req *http.Request) {
	initHeaders(writer)
	api.logger.Info("Get Warehouse by ID /api/v1/product/{id}")
	id, err := strconv.Atoi(mux.Vars(req)["id"])
	if err != nil {
		api.logger.Info("Troubles while parsing {id} param:", err)
		msg := Message{
